...
@protocol CJYCrashCollectDelegate<NSObject>
/**
 监听接收崩溃日志的字符串信息
 例如:
 手机信息:
 ...
 @param crashLog 信息内容
 */
- (void)observerCrashLog:(NSString *)crashLog;

/**
 监听接收崩溃日志的字典信息,根据自己需要的内容自己提取信息
 例如:
 {
 ...
 */
- (void)observerCrashLogDictionary:(NSDictionary *)crashLogDictionary;
@end
@interface CJYCrashCollect : NSObject
@property(nonatomic,weak) id <CJYCrashCollectDelegate>delegate;
